...
import controlServer.SwarmServer;
...
public class test_a_star {
	...
	public static void main(String[] args) throws Exception {
		...
		ROVER_10 rover = new ROVER_10();
		rover.run(); // run the rover
	}

	/**
	 * The seed that this program is built on is a chat program example found
	 * here: http://cs.lmu.edu/~ray/notes/javanetexamples/ Many thanks to the
	 * authors for publishing their code examples
	 */

	static class ROVER_10 implements Mover {
		...
		public ROVER_10() {
			...
			sleepTime = 300; // in milliseconds - smaller is faster, but the
								// server
		...
		// sleepTime = 200; // in milliseconds - smaller is faster, but the
		// server
		// // will cut connection if it is too small
		// }
		...
		public void startRoverServer() throws IOException {
			...
			roverPeerNo = 0; // intially 0, since no rovers are connected to it
								// yet.
		...
		/**
		 * open an IO stream for each RoverPeer connected to the host display
		 * all the messages send to the host rover by the peer rover
		 */
		class RoverPeerHandler implements Runnable {
			...
			public void run() {
				...
				try {
					...
					// print all messages send to the host by continuously
					// reading
					...
					while (true) {
						...
						try {
							...
							// when a client connects to the host, the client
							// will
							...
							if (clientMsg.startsWith("listen")) {
								...
								// message will be in the format: "listen
								// <port>"
		...
		public void run() throws IOException, InterruptedException {
			...
			while (true) {
				...
				finder = new AStarPathFinder(aStarMap, 11, false);

				/**
				 * Finds the most optimal path to (targetIndex, targetIndex) on the current aStarMap
				 * 
				 */				
		...
		private void radiation_sensor(String currentCoord, ArrayList<String> radioactiveelementFetch) {
			...
			// extracting the current coordinates and putting into integer
			// variables
			...
			for (String s : radioactiveelementFetch) {
				// extracting the radioactive coordinates and putting into
				// integer variables
				...
				// checking the x value of radioactive coordinate in the scan
				// map
				...
				// checking the y value of radioactive coordinate in the scan
				// map
				// least will be 0 and max will 10 while 5 will be median
				switch (y_Chemical) {
				case 0:
				...
				if (x_Chemical >= 0 && y_Chemical >= 0) {
					// creating a string form of coordinates to store in
					// arraylist
